...
class tablesResult extends StatefulWidget {
  const tablesResult({Key? key, required this.takenImages, required this. equipType, required this. eqId, required this. stId}) : super(key: key);
  ...
  @override
  State<tablesResult> createState() => _tempImageState();
}

// ignore: camel_case_types
class _tempImageState extends State<tablesResult> {
  ...
  Future<void> fetchDataAndBuildContainers() async {
    for (int i = 0; i < widget.takenImages.length; i++) {
      Uint8List imageData = Uint8List.fromList(widget.takenImages[i]);
      ...
      setState(() {
        x = MediaQuery.of(context).size.width / imageWidth;
        y = MediaQuery.of(context).size
            .height / imageHeight;
      });
      print("$x....$y");
    }
    List<Widget> newContainers = await fetchBoundingBoxes();
    setState(() {
  ...
  Future<List<BoundingBox>> fetchBoundingBox() async {
    try {
      HttpClient client = HttpClient(context: await globalContext);
      client.badCertificateCallback = (X509Certificate cert, String host, int port) => false;
      IOClient ioClient = IOClient(client);
      Uri url = Uri.parse("https://smh-app.trent-tata.com/flask/get_compliance_for_text/257/27");
  ...
  Future<List<Widget>> fetchBoundingBoxes() async {
    try {
      HttpClient client = HttpClient(context: await globalContext);
      client.badCertificateCallback = (X509Certificate cert, String host, int port) => false;
      IOClient ioClient = IOClient(client);
      ...
      var response = await ioClient.get(url);
      if (response.statusCode == 200) {
        var resultsJson = json.decode(response.body).cast<
            Map<String, dynamic>>();
        int counter = 1;
        List<Widget> newContainers = [];

        List<BoundingBoxContainer> boundingBoxes = resultsJson.map<BoundingBoxContainer>((json) {
          List<int> positionValues = parsePositions(json["positions"]);

          BoundingBoxContainer boundingBoxContainer = BoundingBoxContainer(
            positionLeft: positionValues[0]?.toDouble() ?? 0.0,
            positionTop: positionValues[1]?.toDouble() ?? 0.0,
            positionRight: positionValues[2]?.toDouble() ?? 0.0,
            positionBottom: positionValues[3]?.toDouble() ?? 0.0,
          ...
          newContainers.add(
            Positioned(
              top: boundingBoxContainer.positionTop * y,
              left: boundingBoxContainer.positionLeft * x,
              child: GestureDetector(
                onTap: () {
                  handleContainerTap(counter - 1, boundingBoxContainer);
                },
                child: Container(
                  width: boundingBoxContainer.positionWidth * x,
                  height: boundingBoxContainer.positionHeight * y,
                      ...
                      children: [
                        Container(
                          ...
                          child:  const Column(
                              ...
                              children: [
                                // Text(boundingBox.status == 1 ? '✔' : '✘', style: TextStyle(
                                ...
                                // ),),
                                // Icon(
                                //   boundingBox.status == 1
                                //       ? Icons.check
                                //       : boundingBox.status == 5
                                //       ? Icons.circle_outlined
                                //       : Icons.close,
                                //   size: 40,
                                //   color: boundingBox.status == 1
                                //       ? Colors.green
                                //       : boundingBox.status == 5
                                //       ? Colors.white
                                //       : Colors.red,
  ...
  void handleContainerTap(int index, BoundingBoxContainer boundingBox) {
    // Print details when a container is tapped
    print("Index: $index");
    print("Product Code: ${boundingBox.product_code}");
    print("Color: ${boundingBox.color}");
    print("Size Ratio: ${boundingBox.size_ratio}");
    print("Detected Quantity: ${boundingBox.detected_quantity}");
    print("Position: ${boundingBox.position}");
    print("Product: ${boundingBox.product}");
    print("Status: ${boundingBox.status}");
  }
        ...
        text: TextSpan(
          ...
          children: [
            for (String line in boundingBox.product_code.split('\n'))
               TextSpan(
                text: '\n\t\t\tProd: ${boundingBox.product == 1 ? 'Yes\t\t\t' : 'No\t\t\t'}',
            ...
            TextSpan(
              text: '\n\t\t\tPosi: ${boundingBox.position == 1 ? 'Yes\t\t\t' : 'No\t\t\t'}',
              style: const TextStyle(
                color: Colors.white,
                fontSize: 6.0,
                fontWeight: FontWeight.bold,
            ...
            TextSpan(
              text: '\n\t\t\tCol: ${boundingBox.color == 1 ? 'Yes\t\t\t' : 'No\t\t\t'}',
              style: const TextStyle(
                  color: Colors.white,
                  fontSize: 6.0,
                  fontWeight: FontWeight.bold
            ...
            TextSpan(
              text: '\n\t\t\tQty: ${boundingBox.detected_quantity }\t\t\t',
              style: const TextStyle(
                  color: Colors.white,
                  fontSize: 6.0,
                  fontWeight: FontWeight.bold
            ...
            TextSpan(
              text: '\n\t\t\tSize Rt: ${boundingBox.size_ratio == 1 ? 'Yes\t\t\t' : 'No\t\t\t'}',
              style: const TextStyle(
                  color: Colors.white,
                  fontSize: 6.0,
                  fontWeight: FontWeight.bold,
