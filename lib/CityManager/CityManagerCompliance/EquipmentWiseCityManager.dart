...
import '../../model.dart';
import 'ProductWiseCityMangerCompliance.dart';

class EquipmentWiseCitymanager extends StatefulWidget {
  const EquipmentWiseCitymanager({Key? key, this.date, this.storeId, required this. attempt, required this. compliant}) : super(key: key);
  final String? date;
  final String? storeId;
  final String? attempt;
  final String? compliant;
  @override
  State<EquipmentWiseCitymanager> createState() => _HomePageState();
}

class _HomePageState extends State<EquipmentWiseCitymanager> {
                            ...
                            return DataRow(
                                ...
                                cells: [
                                    ...
                                    GestureDetector(
                                      ...
                                      onTap: () {
                                        ...
                                        Navigator.push(
                                          context,
                                          MaterialPageRoute(
                                            builder: (context) => ProductWiseCitymanager(storeId: widget.storeId,date: widget.date ,equipId: data.equipment_id.toString(),
                                  ...
                                  DataCell(
                                      GestureDetector(
                                        child:  Center(
                                          child: Text(data.inserted_on.toString().substring(0, data.inserted_on.toString().length - 3),style: const TextStyle(fontSize: 15,color: Colors.black,)),
                                        ),
                                        // width:40,
                                        onTap: () {
                                          DateTime dateTime = DateFormat("EEE, dd MMM yyyy HH:mm:ss 'GMT'").parseUtc(data.inserted_on.toString());
  ...
  Future<List<CityManagerEquipmentWise>> fetchResults() async {
    ...
    final response = await ioClient.post(
        ...
        body: json.encode({"date":widget.date.toString(),"store_id":widget.storeId.toString(),"attempt":widget.attempt,"compliant":widget.compliant.toString()}),
        headers: {
