...
import 'BillingFeedBack.dart';
import 'TrialRoomFeedBack.dart';
import 'googleReviews.dart';

class googlefeedback extends StatefulWidget {
  const googlefeedback({Key? key, this. stId, required this.newSelectedYear, required this.newSelectedMonth,}) : super(key: key);
  final String? stId;
  final int newSelectedYear;
  final int newSelectedMonth;
  @override
  State<googlefeedback> createState() => _HomePageState();
}
DateTime currentDate = DateTime.now();
//String selectedMonth = DateFormat('MMMM').format(currentDate);
class _HomePageState extends State<googlefeedback> {
  List<dynamic> data = [];
  ...
  Future<void> fetchGoogle() async {
    ...
    Uri url = Uri.parse("https://smh-app.trent-tata.com/flask/getGoogleReviewTableData/$StoreCode/$selectedYear/$selectedMonthName");
    var response = await ioClient.get(url);
    // print("https://smh-app.trent-tata.com/flask/getGoogleReviewTableData/$StoreCode/$selectedYear/$selectedMonthName");
    // print(StoreCode);
    // print(selectedYear);
    // print(selectedMonth);
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        total_review_medium = (data.first['total_review_average'].toString());
            ...
            children:  [
              ...
              FittedBox(
                  ...
                  child: DataTable(
                    ...
                    columns: const [
                      DataColumn(label: Text('Ratings',style:TextStyle(fontWeight: FontWeight.bold,fontSize: 15,color: Colors.black),)),
                      DataColumn(label: Text('Total Reviews',style:TextStyle(fontWeight: FontWeight.bold,fontSize: 15,color: Colors.black),)),
                      DataColumn(label: Text('Total Good\nReviews',style:TextStyle(fontWeight: FontWeight.bold,fontSize: 15,color: Colors.black),)),
                      DataColumn(label: Text('Total Bad\nReviews',style:TextStyle(fontWeight: FontWeight.bold,fontSize: 15,color: Colors.black),)),
                      DataColumn(label: Text('Percentage',style:TextStyle(fontWeight: FontWeight.bold,fontSize: 15,color: Colors.black,),)),
                  ...
                  rows: [
                     DataRow(cells: [
                              ...
                              const Center(
                                  ...
                                  onTap:(){
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                        builder: (context) => GoogleReviews(storeCode:StoreCode.toString(),year:selectedYear.toString(),month:selectedMonth.toString(),rating:'1'),
                                      ),
                                    );
                                  }
                          ),
                          DataCell(Center(
                           ...
                           DataCell(const Center(child:Text("0", textAlign: TextAlign.center, style:  TextStyle(fontSize: 15),)),
                               onTap:(){
                           Navigator.push(
                           context,
                          MaterialPageRoute(
                    ...
                    DataRow(cells: [
                           DataCell(const Center(child:Text("2", textAlign: TextAlign.center, style:  TextStyle(fontSize: 15),)),
                       onTap:(){
                       Navigator.push(
                         context,
                       MaterialPageRoute(
                      builder: (context) => GoogleReviews(storeCode:StoreCode.toString(),year:selectedYear.toString(),month:selectedMonth.toString(),rating:'2'),
                      ),
                          );
                       }
                      ),
                    ...
                    DataRow(cells: [
                    DataCell(const Center(child:Text("3", textAlign: TextAlign.center, style:  TextStyle(fontSize: 15),)),
                          onTap:(){
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => GoogleReviews(storeCode:StoreCode.toString(),year:selectedYear.toString(),month:selectedMonth.toString(),rating:'3'),
                              ),
                            );
                          }),
                    ...
                    DataRow(cells: [
                      DataCell(const Center(child:Text("4", textAlign: TextAlign.center, style:  TextStyle(fontSize: 15),)),
                          onTap:(){
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => GoogleReviews(storeCode:StoreCode.toString(),year:selectedYear.toString(),month:selectedMonth.toString(),rating:'4'),
                              ),
                            );
                          }),
                    ...
                    DataRow(cells: [
                      DataCell(const Center(child:Text("5", textAlign: TextAlign.center, style:  TextStyle(fontSize: 15),)),
                          onTap:(){
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => GoogleReviews(storeCode:StoreCode.toString(),year:selectedYear.toString(),month:selectedMonth.toString(),rating:'5'),
                              ),
                            );
                          }),
