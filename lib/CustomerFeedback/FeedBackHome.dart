...
class SentimentHome extends StatefulWidget {
  const SentimentHome({Key? key,  this.stid, required this.newSelectedYear,}) : super(key: key);
  final String? stid;
...
class _SentimentHomeState extends State<SentimentHome> {
  ...
  Future<void> fetchTrialRoomData() async {
    ...
    client.badCertificateCallback = (X509Certificate cert, String host, int port) => false;
    IOClient ioClient = IOClient(client);
    Uri url = Uri.parse('https://smh-app.trent-tata.com/flask/getTrialRoomSentimentCounts/$StoreCode/$selectedYear/$selectedMonth');
    print("https://smh-app.trent-tata.com/flask/getTrialRoomSentimentCounts/$StoreCode/$selectedYear/$selectedMonth");
    final response = await ioClient.get(url);
    var resultsJson = json.decode(response.body).cast<Map<String, dynamic>>();
    print(resultsJson);
    if (response.statusCode == 200) {
  ...
  Future<void> fetchGoogleData() async {
    ...
    client.badCertificateCallback = (X509Certificate cert, String host, int port) => false;
    IOClient ioClient = IOClient(client);
    Uri url = Uri.parse("https://smh-app.trent-tata.com/flask/getGoogleReviewTableData/$StoreCode/$selectedYear/$Month");
    //print("https://smh-app.trent-tata.com/flask/getGoogleReviewTableData/$StoreCode/$selectedYear/$Month");
                ...
                Container(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      ...
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => BillingFeedback(
                            stId:widget.stid,
             ...
             child: Text('Data is empty')),
               ...
               :  FittedBox(
                  ...
                  // columnSpacing: 100,
                   ...
                   Colors.grey.shade200),
                    columns: const [
                   DataColumn(label: Text('SENTIMENT' ,style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.black))),
                   DataColumn(label: Text('BILLING', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.black))),
                   DataColumn(label: Text('TRIAL', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.black))),
                   DataColumn(label: Text('GOOGLE', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.black))),
              ],
              ...
              rows: [
                ...
                DataRow(cells: [
                  DataCell(Container(
                      width:50,
                      child: const Text('SCORE', textAlign: TextAlign.center))),
                  DataCell(
                    Container(
                      width:40,
