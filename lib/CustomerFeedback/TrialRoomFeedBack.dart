...
class TrialRoomFeedback extends StatefulWidget {
  const TrialRoomFeedback({Key? key, this.stId, required this.newSelectedYear, required this.newSelectedMonth,}) : super(key: key);
  final String? stId;
  final int newSelectedYear;
  final int newSelectedMonth;
  @override
  _TrialRoomFeedbackState createState() => _TrialRoomFeedbackState();
}

class _TrialRoomFeedbackState extends State<TrialRoomFeedback> {
  List<dynamic> data = [];
  List<dynamic> lmdata = [];
  List<dynamic> lydata = [];
  List<dynamic> stores = [];
  ...
  Future<void> fetchData() async {
    ...
    Uri url = Uri.parse(
        'https://smh-app.trent-tata.com/flask/getTrialRoomSentimentCounts/$StoreCode/$selectedYear/$selectedMonth');
  ...
  Future<void> fetchLMData(selectedMonth) async {
    ...
    IOClient ioClient = IOClient(client);
    Uri url = Uri.parse(
        'https://smh-app.trent-tata.com/flask/getLMTrialRoomSentimentCounts/$StoreCode/$selectedYear/$lastMonth');
    var response = await ioClient.get(url);
    var resultJson = json.decode(response.body);
    print("$resultJson.........fetchdata");
    if (response.statusCode == 200) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: const Text('Voice of Customer TrialRoom Table',
                    ...
                    setState(() {
                      ...
                      if (selectedTypeOfFeedback == "BILLING") {
                        Navigator.pushReplacement(
                          ...
                          MaterialPageRoute(
                            builder: (context) =>
                                BillingFeedback(stId: widget.stId,
                                  newSelectedYear: selectedYear, // Use selectedYear
                                  newSelectedMonth: selectedMonth,),
                          ),
                        );
                        ...
                        DataRow(cells: [
                          ...
                          DataCell(
                            Container(
                              ...
                              child: Row(
                                children: [
                                  Text(good_cleanliness_count.toString(),
                                      textAlign: TextAlign.left),
                                ],
                              ),
                            ),
                        ...
                        DataRow(cells: [
                          ...
                          DataCell(
                            Container(
                              ...
                              child: Row(
                                children: [
                                  Text(medium_cleanliness_count.toString(),
                                      textAlign: TextAlign.left),
                                ],
                              ),
                            ),
                        ...
                        DataRow(cells: [
                          ...
                          DataCell(
                            Container(
                              ...
                              child: Row(
                                children: [
                                  Text(low_cleanliness_count.toString(),
                                      textAlign: TextAlign.left),
                                ],
                              ),
                            ),
